...
editor_html[i]=CodeMirror.fromTextArea(id,{mode:'text/html',lineNumbers:true,htmlMode:true,});var code=editor_html[i].getValue();practiceResult(i,code);});$('.practice_run').click(function(){var n=$(this).data('number');var code=editor_html[n].getValue();practiceResult(n,code);checkCode(n,code);});$('.practice_answer').click(function(){var n=$(this).data('number');var showAnswer=$(this).data('answer');if(showAnswer===true){$('#practice_result-'+n).hide();$('#practice_run-'+n).prop('disabled',false);$(this).html('<i class="icon-help"></i> Показать ответ');$(this).data('answer',false);}else if(showAnswer===false){$('#practice_result-'+n).show();$('#practice_run-'+n).prop('disabled',true);$('#practice_answer-'+n).html('<i class="icon-help"></i> Спрятать ответ');$(this).data('answer',true);}});var iframe=[],iframeContent=[];function practiceResult(n,code){var iframe=document.createElement('iframe');var codeFull=checkDoctype(code);document.getElementById('practice_browser-'+n).innerHTML="";document.getElementById('practice_browser-'+n).appendChild(iframe);var iframeContent=(iframe.contentWindow)?iframe.contentWindow:(iframe.contentDocument.document)?iframe.contentDocument.document:iframe.contentDocument;iframeContent.document.open();iframeContent.document.write(codeFull);iframeContent.document.close();}
...
function checkHTML(n,code){result=$('#practice_result-'+n).text();result=result.replace(/(\r\n|\n|\r)/gm," ");code=code.replace(/(\r\n|\n|\r)/gm," ");result=result.replace(/ |\t/gm,"");code=code.replace(/ |\t/gm,"");result=result.toLowerCase();code=code.toLowerCase();if(result==code)return 1;else return 0;}
function checkDoctype(code){var doctype=code.toLowerCase().indexOf('<!doctype');if(doctype==-1){code='<!DOCTYPE html><html><head><meta charset="utf-8"><title>webref.ru</title><base target="_blank" href="/example/"></head><body>'+code+'</body></html>';}
return code;}})(jQuery);;
